...
import os
import codegen_game as codegen
import torch
...
class Loader:
    ...
    def initialize(self):
        ...
        desc["actor"] = dict(
            ...
            input=dict(T=1, keys=set(["mem_in", "mem_out", "code", "last_terminal"])),
        ...
        if not args.actor_only:
            ...
            desc["train"] = dict(
                batchsize=args.batchsize,
                input=dict(T=1, keys=set(["rv", "id", "pi", "mem_in", "mem_out", "code", "a", "V", "seq", "last_terminal"])),
                reply=None
            )
...
cmd_line = "--num_games 64 --batchsize 16 --actor_only"
